...
public class Debug {
    ...
    static {
        Starting_Items = new ArrayList<>(Arrays.asList(
                ...
                TimekeepersHourglass.class, EtherealChains.class,
                WandOfBlastWave.class, WandOfCorrosion.class, WandOfFireblast.class
        ));
    }
    public static void Starting_Bag() {
        ...
        ClothArmor cloth_flow = (ClothArmor)DebugCreate(ClothArmor.class, 0, 1);
        cloth_flow.inscribe(Reflection.newInstance(Flow.class));
        cloth_flow.collect();

        if(Dungeon.hero.heroClass == HeroClass.HUNTRESS) {
            SpiritBow bow = (SpiritBow)DebugCreate(SpiritBow.class, 0, 1);
            bow.enchant(Reflection.newInstance(Blazing.class));
            bow.collect();
        } else {
            Shortsword sword = (Shortsword)DebugCreate(Shortsword.class, 0, 1);
            sword.enchant(Reflection.newInstance(Blazing.class));
            sword.collect();
        }
    }
